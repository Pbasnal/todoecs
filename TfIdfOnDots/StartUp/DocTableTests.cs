...
public class DocTableTests : Tests
{
    ...
    public DocTableTests()
    {
        testsToRun = new List<Action>{
            ...
            //RemoveDocuments,
            // AddTokenFrequencies,
            // RemoveTokenFrequencies
            // TokensTableTestsInsertTokens
            TokensTableTestsRemoveTokens
        };
    }
    ...
    private void AddTokenFrequencies()
    {
        DocumentTokensOps tfOps = new();

        Dictionary<string, string> documents = new Dictionary<string, string>{

            {"doc1", "the the another game"},
            {"doc2", "elden ring is the story of all story"}
            // {"doc1", "The Dirtmouth town in Hollow Knight has a charm eerily similar to Ashina's outskirts in Sekiro."},
            // {"doc2", "Cross-references are noted between anime 'Attack on Titan' and Elden Ring's cathedral of the deep."},
            // {"doc3", "Elden Ring's universe pulls a 'Fullmetal Alchemist' with its transmutation-based magic system."},
            // {"doc4", "Sekiro's Headless apes share a uncanny resemblance with 'Bleach's' Menos Grande Hollows."},
        ...
        foreach (string documentName in documents.Keys)
        {
            tfOps.UpdateTermsOfDocument(index, documents[documentName]);
            index += 1;
        }

        tfOps.PrintAllTokenFrequencies();
    }

    private void RemoveTokenFrequencies()
    {
        DocumentTokensOps tfOps = new();

        Dictionary<string, string> documents = new Dictionary<string, string>{
            {"doc1", "the the another game"},
            {"doc2", "elden ring is a story A story of all story"}
            // {"doc1", "The Dirtmouth town in Hollow Knight has a charm eerily similar to Ashina's outskirts in Sekiro."},
            // {"doc2", "Cross-references are noted between anime 'Attack on Titan' and Elden Ring's cathedral of the deep."},
            // {"doc3", "Elden Ring's universe pulls a 'Fullmetal Alchemist' with its transmutation-based magic system."},
            // {"doc4", "Sekiro's Headless apes share a uncanny resemblance with 'Bleach's' Menos Grande Hollows."},
        ...
        Console.WriteLine("\nRemoving tokens");
        tfOps.RemoveTokensOfDocument(0);

        tfOps.PrintAllTokenFrequencies();
    }

    private void TokensTableTestsInsertTokens()
    {
        ...
        tokenOps.PrintAllTokens();

    }

    private void TokensTableTestsRemoveTokens()
    {
        TokensTableOps tokenOps = new();
